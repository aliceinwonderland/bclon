...
La clonaci'on se ha aplicado principalmente para desarrollar controladores usando las trazas de control de operadores expertos. La idea es reconstruir la estrategia de control que el operador ejecuta subconscientemente.
\begin{itemize}
\item Simuladores de vuelo. La idea es que al obtener trazas de pilotos expertos, el simulador pueda realizar un vuelo de forma autom'atica de forma parecida a como lo hace el piloto. Tambi'en se buscaba obtener el modelo de forma expl'icita para poder entender esa estrategia. Lo que se hizo fu'e tomar trazas de pilotos expertos, procesarlas con algoritmos de inducci'on de 'arboles. Se obtuvieron modelos que se pod'ian ejecutar en el simulador. Una de los usos que se le di'o fue para entrenar a pilotos.
\item El p'endulo invertido. El objetivo es extraer un modelo que al ejecutarlo permita mantener el p'endulo en equilibrio. Extraer un modelo entendible es tambi'en importante.
